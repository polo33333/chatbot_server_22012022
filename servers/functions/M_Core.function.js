...
const IntentModel = require('../models/Intent.model');
...
exports.handlePostbackPlaform = async (sender_id, message, channel, channel_token, botId) => {
    ...
    try {
        ...
        if (obj.redirectToBlock != null) {
            ...
            //     await saveVariable(mem._id, [{ _id: obj.key, value: obj.value }], false);
            //     //console.log(v)
            // }
...
//handle comments for feed
exports.handleFeedFace = async (fromId, post_id, commentId, message, channel_token, botId) => {

    try {


        let conf = await Config.findOne({ botId: botId });
        if (fromId != conf.pageId) {
            let wit_object = await wit_understanding(message, botId);
            if (wit_object != null) {
                if (wit_object.intents.length != 0) {
                    ...
                    if (intents.length == 0) {
                        ...
                        let r = await Comment.findOne({ botId: botId, isActive: true, pageId: conf.pageId, feedId: post_id });
                        if (r)
                            await FaceBook.reply(commentId, r.items[0].content, channel_token);
                    ...
                    if (intents.length == 1) {
                        ///console.log(1)
                        let int = await Intent.findOne({ name: intents[0].name, botId: botId });
                        let r = await Comment.findOne({ botId: botId, isActive: true, pageId: conf.pageId, feedId: post_id });
                        let re = r.items.filter(x => x.intentName == int._id);
                        if (re)
                            await FaceBook.reply(commentId, re[0].content, channel_token);
                        else
                            await FaceBook.reply(commentId, r.items[0].content, channel_token);
                        return;
                    }
                    ...
                    let r = await Comment.findOne({ botId: botId, isActive: true, pageId: conf.pageId, feedId: post_id });
                    if (r)
                        return await FaceBook.reply(commentId, r.items[0].content, channel_token);
                }
        ...
        console.log('Error[M_Core:handleFeed]: ' + error);
        return null;
    }
}
                    ...
                    case C_String.form_card:
                        {
                            ...
                            if (is_mapping) {
                                if (Validate(mem.works[0].validation, message)) {
                                    let value = await wit_value_checking(mem.works[0].validation, mem.works[0].variable_id, message, botId)
                    ...
                    case C_String.phone_card:
                        {
                            ...
                            if (Validate(mem.works[0].items[0].validation, message)) {
                                if (mem.works[0].items[0].saveToVariable) {
                                    saveVariable(memoryId, [{ _id: mem.works[0].items[0].saveToVariable, value: message.replace(/\s/g, '') }], false);
                                }
                                mem.works.shift();
