...
const M_SpeechToText = require('../functions/M_SpeechToText.function');
...
exports.webhookVerify = async (req, res) => {
    ...
    if (mode && token) {
        ...
        if (mode === 'subscribe' && token === config.fa_verify_Code) {
...
exports.webhook = async (req, res) => {
    ...
    var body = req.body;
    var currConfig = await Config.findOne({ "fa_linked_page.id": body.entry[0].id });
        ...
        body.entry.forEach(function (entry) {
            ...
            if (entry.changes != undefined) {
                ...
                if (webhook_event_page.field == "feed" && webhook_event_page.value.item == "comment" && webhook_event_page.value.verb == "add") {
                    handleFeed(webhook_event_page.value.from.id, webhook_event_page.value.post_id, webhook_event_page.value.comment_id, webhook_event_page.value.message, currConfig.fa_linked_page.access_token, currConfig.botId);
            ...
            if (entry.messaging != undefined) {
                ...
                if (webhook_event.message) {
                    ...
                    handleMessage(currConfig.botId, currConfig.fa_linked_page.access_token, sender_psid, webhook_event.message);
                } else if (webhook_event.postback) {
                    //console.log(webhook_event.postback)
                    handlePostback(currConfig.botId, currConfig.fa_linked_page.access_token, sender_psid, webhook_event.postback);
                }
            }
...
exports.sendMessage = async (senderId, temp_message, PAGE_ACCESS_TOKEN, botId, type) => {
    ...
    if (type != undefined && type == "broadcast")
        sendObject.tag = "CONFIRMED_EVENT_UPDATE";
...
handleMessage = async (botId, PAGE_ACCESS_TOKEN, sender_psid, received_message) => {
    ...
    if (received_message.text) {
        ...
        if (received_message.attachments != undefined) {
            if (received_message.attachments[0].type == 'audio') {
                await M_SpeechToText.quickstart(sender_psid, C_String.face, PAGE_ACCESS_TOKEN, botId, received_message.attachments[0].payload.url);
            }
        }
...
face_media_template = (senderId, temp_message) => {
    ...
    sendObject.message.attachment.payload.elements[0].image_url = config.host_url + temp_message.image_url;
    if (temp_message.button.length > 0)
